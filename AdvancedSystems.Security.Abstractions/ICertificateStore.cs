...
public interface ICertificateStore : IDisposable
{
    ...
    /// <summary>
    ///     Opens an X.509 certificate store or creates a new store, depending on <seealso cref="OpenFlags"/>
    ///     flag settings.
    /// </summary>
    /// <param name="flags">
    ...
    ///     Thrown when the store cannot be opened as requested.
    /// </exception>
    /// <exception cref="SecurityException">
    ///     Thrown when the caller does not have the required permission.
    /// </exception>
    /// <exception cref="ArgumentException">
    ///     Thrown when the store contains invalid values.
    /// </exception>
    /// <remarks>
    ///     Use this method to open an existing X.509 store. Note that you must have additional permissions,
    ///     specified by <c>StorePermissionFlags</c>, to enumerate the certificates in the store. You can
    ///     create a new store by passing a store name that does not exist to the class constructor, and
    ///     then using any of the <seealso cref="OpenFlags"/> flags except <seealso cref="OpenFlags.OpenExistingOnly"/>.
    /// </remarks>
